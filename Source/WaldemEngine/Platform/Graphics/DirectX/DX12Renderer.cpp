...
namespace Waldem
{
    ...
    void DX12Renderer::InitializeBindless()
    {
        ...
        Device->CreateCommandSignature(&commandSigDesc, GeneralRootSignature, IID_PPV_ARGS(&CommandSignature));
    ...
    void DX12Renderer::DrawIndirect(uint numCommands, Buffer* indirectBuffer, bool isIndexed)
    {
        SetRenderTargets();
        
        auto& cmd = WorldCommandList.first;
        ID3D12Resource* resource = ResourceMap[(GraphicResource*)indirectBuffer];
        cmd->DrawIndirect(isIndexed ? IndexedCommandSignature : CommandSignature, numCommands, resource);
    }
    ...
    void DX12Renderer::BeginUI()
    {
        ...
        if(Input::IsCursorCapturedForGameplay())
        {
            ImGuiIO& io = ImGui::GetIO();

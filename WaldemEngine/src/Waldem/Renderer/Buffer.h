...
namespace Waldem
{
    ...
    struct BufferElement
    {
        ...
        BufferElement() {}
        BufferElement(ShaderDataType type, const WString& name, bool normalized) : Type(type), Name(name), Size(ShaderDataTypeSize(type)), Offset(0), Normalized(normalized)
        {
        }
    ...
    {
    public:
        Buffer(WString name, BufferType type, size_t size, uint32_t stride) : Name(name), Type(type), Size(size), Stride(stride) {}
        virtual ~Buffer() {}

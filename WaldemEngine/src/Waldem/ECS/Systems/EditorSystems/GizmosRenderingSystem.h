...
namespace Waldem
{
    ...
    class WALDEM_API GizmosRenderingSystem : public ICoreSystem
    {
    private:
        // Quad geometry
    ...
    public:
        ...
        void Initialize() override
        {
            ...
            // Create 2 materials (camera/light)
            ...
            ECS::World.system("GizmoDrawSystem").kind(flecs::OnDraw).run([&](flecs::iter& it)
            {
                DrawIcons();
            });
        }
        ...
        void RemoveIcon(flecs::entity e)
        {
            ...
            {
                uint packedCamera = ((uint)gizmoId << 16) | (uint)GizmoCamera;
                uint packedLight  = ((uint)gizmoId << 16) | (uint)GizmoPointLight;
        ...
        void DrawIcons()
        {
            ...
            if(viewport->TryGetLinkedCamera(linkedCamera))
            {
                ...
                auto deferredRT = gbuffer->GetRenderTarget(Deferred);
                ...
                Renderer::BindRenderTargets({ deferredRT });

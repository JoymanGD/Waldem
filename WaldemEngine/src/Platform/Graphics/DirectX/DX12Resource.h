...
namespace Waldem
{
    struct ResourceData
    {
        ...
        GraphicResource Desc;
        uint32_t RootParameterIndex = 0;
        D3D12_RESOURCE_STATES CurrentState = D3D12_RESOURCE_STATE_COMMON;

        ResourceData(GraphicResource desc, uint32_t rootParameterIndex, D3D12_RESOURCE_STATES initialState)
            : Desc(desc), RootParameterIndex(rootParameterIndex), CurrentState(initialState)
        {
        }

        ResourceData(ID3D12Resource* uploadResource, ID3D12Resource* defaultResource, ID3D12Resource* readbackResource, GraphicResource desc, uint32_t rootParameterIndex, D3D12_RESOURCE_STATES initialState)

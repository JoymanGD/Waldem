...
#define MAX_BUFFERS 128
    ...
    DX12RootSignature::DX12RootSignature(ID3D12Device* device, DX12CommandList* cmdList, WArray<GraphicResource> resources) : CmdList(cmdList)
    {
    ...
    void DX12RootSignature::SetResources(ID3D12Device* device, DX12CommandList* cmdList, WArray<GraphicResource> resourceDescs, uint32_t numDescriptors)
    {
        if(numDescriptors == 0)
        {
...
#pragma endregion
    ...
    void DX12RootSignature::ClearResource(WString name)
    {
        auto resourceData = ResourcesMap[name];
    ...
    void DX12RootSignature::ReadbackResourceData(WString name, void* destinationData)
    {
        if(destinationData)
        {
            auto resourceData = ResourcesMap[name];

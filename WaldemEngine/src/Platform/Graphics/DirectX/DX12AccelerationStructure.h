...
namespace Waldem
{
    ...
    {
    public:
        DX12AccelerationStructure(WString name, ID3D12Device5* device, DX12CommandList* cmdList, AccelerationStructureType type, WArray<RayTracingGeometry>& geometries);
        DX12AccelerationStructure(WString name, ID3D12Device5* device, DX12CommandList* cmdList, AccelerationStructureType type, WArray<RayTracingInstance>& instances);

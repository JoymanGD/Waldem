...
namespace Waldem
{
    DX12RenderTarget::DX12RenderTarget(WString name, ID3D12Device* device, DX12CommandList* cmdList, int width, int height, TextureFormat format)
        : RenderTarget(name, width, height, format)
    {
        ...
        if(format == TextureFormat::D32_FLOAT)
        {
            ...
            device->CreateDepthStencilView(Resource, &dsvDesc, RTVHandle);
        }
        else
        {
    ...
    DX12RenderTarget::DX12RenderTarget(WString name, ID3D12Device* device, DX12CommandList* cmdList, int width, int height, TextureFormat format, ID3D12DescriptorHeap* srvHeap, uint slot)
        : RenderTarget(name, width, height, format)
    {
        HRESULT hr;
    ...
    DX12RenderTarget::DX12RenderTarget(WString name, ID3D12Device* device, int width, int height, TextureFormat format, ID3D12Resource* resource)
    {
        Resource = resource;
